...
class ArticleTableViewController: UITableViewController {
    ...
    private func fetchData(atPage: Int, withLimitation: Int) {
        self.articleViewModel?.getArticle(atPage: atPage, withLimitation: withLimitation) {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if self.articleViewModel?.articleViewModel != nil {
            if (articleViewModel?.articleViewModel.count)! > 0 {
                self.tableView.separatorStyle = .singleLine
            } else {
                self.tableView.separatorStyle = .none
                return (articleViewModel?.articleViewModel.count)!
            }
            return (articleViewModel?.articleViewModel.count)!
        }
